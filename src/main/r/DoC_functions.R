...
### 4c: normalize vs. median using invariant set methods ###
		...
		sd.diff.coverage <- sd(normalized.coverage-as.numeric(sample.coverage[,"MedianCoverage"]))
...
### 4e: generate final coverage data and output figures ###
		...
		png(file=paste(sample.id, "HistogramPlot.png", sep="_"), width=11, height=8, units="in", res=1000)
		par(mfrow = c(1, 3))
...
### 4g: generate QC output ###
		...
		write.table(qc.table.final, paste(criteria$experiment.name, "_QCTable.csv", sep=""), sep=",", col.names=T, row.names=F, quote=F)
		   ...
		   print(paste("Failed analysis for ", sample.id))
		})
	}
...
#		print(date())
		...
		fasta.info(paste(partition.chr, ".fa", sep=""), seqtype="DNA")
		tmp.partition.seq <- readDNAStringSet(paste(partition.chr, ".fa", sep=""))
		partition.seq <- tmp.partition.seq[[1]]
		#partition.seq <- DNAString(readFASTA(paste(partition.chr, ".fa", sep=""), strip.descs=TRUE)[[1]]$seq)
...
"olivedrab1", "orangered", "deepskyblue", "green", "deeppink3")
	...
	setwd(paste(criteria$experiment.directory, "PDFs", sep="/"))
	png(file=paste(sample.id, "RatioPlot.png", sep="_"), width=11, height=8, units="in", res=1000)
	par(mfrow=c(2,1))
	...
	abline(100,0)
	abline(0,0)
	position <- 1:nrow(sample.coverage)
	for (gene.index in 1:length(gene.names)) {
		sample.rows <- which(gene.region==gene.names[gene.index])
		points(position[sample.rows], sample.coverage[sample.rows, "Coverage"], type="h", cex=.1, pch=16, col=color.scheme[gene.index], lwd=.1)
		}
	...
	for (gene.index in 1:length(gene.names)) {
		sample.rows <- which(gene.region==gene.names[gene.index])
		points(position[sample.rows], sample.coverage[sample.rows, "RatioNormalized"], type="p", cex=.1, pch=16, col=color.scheme[gene.index])
		}
	legend("topleft", legend=experiment.metadata$partition.information[,"PartitionName"], col=color.scheme, horiz=T, cex=(.45*(22/length(gene.names))), pch=15, pt.cex=1, bty="n", bg="white")
	dev.off()
}
...
load.median.data <- function(criteria, experiment.metadata) {
	...
	for (partition.index in 1:nrow(experiment.metadata$partition.information)) {
		partition.name <- experiment.metadata$partition.information[partition.index,1]
		partition.chr <- experiment.metadata$partition.information[partition.index,2]
		partition.coverage <- read.table(paste("Median_", partition.name, ".raw", sep=""), header=T, colClasses=c("character", "character", "numeric", "numeric", "numeric"))
		#partition.coverage <- partition.coverage[, -c(2)]
...
# run for loop covering all samples to analyze
	...
	for (partition.index in 1:nrow(experiment.metadata$partition.information)) {
		...
		for (sample.index in 1:nrow(experiment.metadata$sample.information)) {
			sample.id <- experiment.metadata$sample.information[sample.index,"Sample"]
### load sample data ###
			print(paste("Running on ", sample.id))
		...
		write.table(final.coverage, paste("median_", partition.name, ".raw", sep=""), col.names=T, row.names=F, quote=F)
		median.coverage <- apply(final.coverage[,4:ncol(final.coverage)],1,median)
		sd.coverage <- apply(final.coverage[,4:ncol(final.coverage)],1,sd)
		...
		setwd(paste(criteria$experiment.directory, "PDFs", sep="/"))
		png(file=paste(partition.name, "HistogramPlot.png", sep="_"), width=11, height=8, units="in", res=1000)
		par(mfrow = c(1, 3))
		...
		dev.off()
		png(file=paste(partition.name, "Coverage.png", sep="_"), width=20, height=10, units="in", res=1000)
...
# plot sn coverage #
	...
	write.table(median.coverage, paste("median_cov_", partition.name, ".raw", sep=""), col.names=T, row.names=F, quote=F)
	setwd(paste(criteria$experiment.directory, "PDFs", sep="/"))
	png(file=paste("Experiment", "HistogramPlot.png", sep="_"), width=11, height=8, units="in", res=1000)
	par(mfrow = c(1, 3))
	...
	print("About to read cov")
	#bedgraph.main <- median.coverage[,c("ChrId","Position","MedianCoverage","SDCoverage","SNCoverage")]
	#print("Past")
	#bedgraph.main[,3] <- as.numeric(bedgraph.main[,3]) + 1
	#bedgraph.main <- order.chr(bedgraph.main,1,2)
	#bedgraph.main[,2] <- format(bedgraph.main[,2], scientific=F)
...
load.coverage.data <- function(sample.id, criteria, experiment.metadata) {
	setwd(paste(criteria$experiment.directory, "raw", sep="/"))
	print(criteria$experiment.directory)
	coverage <- matrix(nrow=0, ncol=3)
	for (partition.index in 1:nrow(experiment.metadata$partition.information)) {
		partition.name <- experiment.metadata$partition.information[partition.index,1]
		partition.chr <- experiment.metadata$partition.information[partition.index,2]
...
# rawIntensity is autosome and x chromosome.
	logIntAuto <- log(rawIntensity[1:lastAuto])
	refAuto <- log(rawRef[1:lastAuto])
...
# correct for 0 intensity/coverage measurements
	...
	intNorm <- log(rawIntensity) + log(rawRef) - predict(normalized$n.curve,log(rawRef))$y
...
### Run calling functions for 0 and 4+ copies ###
		...
		### Run an extra pass of calling functions for 1 and 3 copies, meant to catch long cnvs ###
			called.probes.proband <- run.seed.call(
			   test.data, 
			   criteria$long.loss.seed.signal, 
			   criteria$long.gain.seed.signal, 
			   criteria$long.loss.extend.signal, 
			   criteria$long.gain.extend.signal,
			   criteria$long.window.size,
			   criteria$long.pass.count
			   )
			cnv.regions <- call.cnv.regions(ratio.table[,"Position"], called.probes.proband)
			cnv.regions <- event.merge(cnv.regions,5000,.1)
			cnv.region.details.3 <- get.cnv.details(
				as.character(partition.chr), 
				cnv.regions, 
				test.data, 
				ratio.table, 
...
### Combine tables and process calls	
			cnv.region.details <- rbind(cnv.region.details.1, cnv.region.details.2, cnv.region.details.3)	
			...
			if (nrow(cnv.region.details)>0) {
				...
				copy.number <- ifelse(as.numeric(cnv.region.details[,"median.ratio"])>=criteria$long.gain.call.signal,3,copy.number)
...
# test for parent overlap
	...
	} else {gain.result.matrix <- matrix(nrow=0, ncol=3)}
# then call deletions
...
# print(result.matrix[cnv.index,])
	...
	} else {loss.result.matrix <- matrix(nrow=0, ncol=3)}
# then return data
